...
public class AddPlatformServiceFeature extends AbstractAddFeature{
	...
	public AddPlatformServiceFeature(IFeatureProvider fp) {
		super(fp);
	}
...
//	@Override
	public PictogramElement add(IAddContext context) {
		...
		PlatformService platformService = (PlatformService)context.getNewObject();
		...
		if (platformService instanceof DeployableSupportService){
		...
		// TEXT
		{
			...
			Text text = gaService.createText(shape, platformService.getName());
			text.setHorizontalAlignment(Orientation.ALIGNMENT_CENTER);
			text.setVerticalAlignment(Orientation.ALIGNMENT_CENTER);
			
			if (platformService instanceof DeployableRuntimeService){
				text.setForeground(gaService.manageColor(diagram, TEXT_DR_PLATFORM_SERVICE));
			}
			else if (platformService instanceof DeployableSupportService){
				text.setForeground(gaService.manageColor(diagram, TEXT_DS_PLATFORM_SERVICE));
			}
			else if (platformService instanceof ExternalRuntimeService){
				text.setForeground(gaService.manageColor(diagram, TEXT_ER_PLATFORM_SERVICE));
			}
			else if (platformService instanceof ExternalSupportService){
		...
		link(containerShape, platformService);
