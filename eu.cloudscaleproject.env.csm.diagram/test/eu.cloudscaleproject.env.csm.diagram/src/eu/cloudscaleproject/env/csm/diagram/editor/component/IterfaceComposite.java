...
public class IterfaceComposite extends Composite {
	 ...
	 * @param style
	 */
	public IterfaceComposite(Composite parent, PlatformService pService) {
		...
		if(this.pService instanceof OperationInterfaceContainer){
			this.providedInterface = ((OperationInterfaceContainer) this.pService).getProvidedInterfaces().get(0);
		}
		else if(this.pService instanceof ApplicationServiceContainer){
			ApplicationServiceContainer asc = ((ApplicationServiceContainer) this.pService);
			//CSMRefactor start
			if(asc.getApplicationServices().isEmpty()){
				throw(new IllegalArgumentException("Platform service has no OperationInterfaces"));
			}
			this.providedInterface = asc.getApplicationServices().get(0).getProvidedInterfaces().get(0);
			//CSMRefactor END
		}
		
		ResourceSet rset = this.pService.eResource().getResourceSet();  
