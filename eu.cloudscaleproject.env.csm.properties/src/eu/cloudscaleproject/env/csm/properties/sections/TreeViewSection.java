package eu.cloudscaleproject.env.csm.properties.sections;
...
import eu.cloudscaleproject.env.csm.properties.HighlightDecorationProvider;
import eu.cloudscaleproject.env.csm.properties.components.CsmTreeViewEditor;
...
public class TreeViewSection extends GFPropertySection implements ITabbedPropertyConstants{
	...
	public void refresh() {
		...
		Object bo = CsmDiagramUtil.getBusinessObject(getSelectedPictogramElement());
		
		if(bo != null && bo instanceof EObject){
			EObject input = (EObject)bo;
			this.csmTreeEditor.setInput(input);
	    	this.csmTreeEditor.redraw();
	    	this.highlightProvider.setDiagram(getDiagram(), getDiagramEditor());
		}
	}
