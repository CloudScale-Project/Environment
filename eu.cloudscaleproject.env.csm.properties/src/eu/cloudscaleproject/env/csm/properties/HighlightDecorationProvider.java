...
import eu.cloudscaleproject.env.csm.diagram.decorations.DecorationProvider;
...
public class HighlightDecorationProvider implements DecorationProvider{
			...
			if(o instanceof List<?>){
				...
				if(!list.isEmpty() && list.get(0) instanceof OperationInterface){
					for(Object item : list){
						...
						addPictograms(CsmDiagramUtil.getPictogramElementsFromBusinessObjects(diagram, op.getProvidingContainer()), colorSec);
						addPictograms(CsmDiagramUtil.getPictogramElementsFromBusinessObjects(diagram, op.getRequiringContainer()), colorSec);
					}
				}
			}
			else if(o instanceof OperationInterface){
				OperationInterface oi = (OperationInterface)o;
				addPictograms(CsmDiagramUtil.getPictogramElementsFromBusinessObjects(diagram, oi.getProvidingContainer()), colorSec);
				addPictograms(CsmDiagramUtil.getPictogramElementsFromBusinessObjects(diagram, oi.getRequiringContainer()), colorSec);
			}
	...
	private void collectBaseHighlitedElements(TreeRow row){
		...
		addPictograms(CsmDiagramUtil.getPictogramElementsFromBusinessObjects(diagram, o), color);
		...
		if(!highlightedElements.containsValue(color)){
			if(o instanceof ComputingInfrastructureService){
				...
				addPictograms(CsmDiagramUtil.getPictogramElementsFromBusinessObjectsList(diagram, cis.getPlatformServices()), color);
			}
	...
	public void setDiagram(Diagram diagram, IDiagramEditor editor){
	...
	public void dispose(){
		...
		this.treeViewer.removeSelectionChangedListener(selectionListener);
		this.treeViewer.getTree().removePaintListener(paintListener);
